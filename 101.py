...
class DynamicHybridSystem:
    ...
    def calculate_dynamic_power_distribution(self, current_source, solar_generation,
                                             ...
                                             active_sources):
        """Calculate dynamic power distribution based on ACTIVE sources only"""
        ...
        # FIXED: Smart power source recommendation based on actual conditions
        # Calculate optimal source based on conditions
        optimal_source = self.recommend_optimal_source(
            available_solar, available_battery, current_usage, battery_soc
        )

        # Apply optimal source if different from current
        ...
        if current_source == "solar" and 'solar' in active_sources:
            if available_solar >= current_usage:
                ...
                # Excess solar goes to battery charging if battery is active
                if available_solar > current_usage and battery_soc < 100 and 'battery' in active_sources:
                    battery_power = -(available_solar - current_usage)  # Negative for charging
            else:
                solar_power = available_solar
                deficit = current_usage - solar_power
                if available_battery > 0 and battery_soc > 20:
                    # Use battery for deficit
                    battery_power = min(deficit, available_battery * 0.8)
            ...
            if battery_soc > 10:
                battery_power = min(current_usage, available_battery * 0.9)
                ...
                # Battery too low, use other active sources
                if 'solar' in active_sources:
            ...
            # If solar available and active, use it to charge battery if battery is active
            if available_solar > 0 and battery_soc < 90 and 'battery' in active_sources:
                battery_power = -min(available_solar, (100 - battery_soc) / 100 * battery_size)
            ...
            if 'battery' in active_sources and battery_soc > 30:
    ...
    def recommend_optimal_source(self, available_solar, available_battery, current_usage, battery_soc):
        """FIXED: Recommend optimal power source based on actual conditions"""
        # Get current time
        ...
        if battery_soc > 50 and available_solar < current_usage * 0.5:
            return "battery"

        # Rule 4: If battery is very low (<20%), use grid to conserve battery
        if battery_soc < 20:
            return "grid"

        # Rule 5: During night hours, use battery if well charged
        if (current_hour < 6 or current_hour > 18) and battery_soc > 40:
            return "battery"

        # Rule 6: Default to grid if no better option
        return "grid"
...
**Health Indicators:**
- **Optimal Range**: Keep between 20%-90% for longest battery life
...
def update_live_stats():
    """Update live statistics with timezone-aware timestamp"""
...
# ============================================================================
# UPDATED HOUSEHOLD DASHBOARD WITH OPEN-METEO API
# ============================================================================

def create_household_dashboard():
    """PowerBI-style dashboard with power source control and developer mode"""
    ...
    with st.sidebar:
        ...
        grid_status = st.selectbox("⚡ Grid Power Status", ["Stable", "Unstable", "Load Shedding", "No Power"], index=1)
    ...
    with col4:
        ...
        is_day = weather_data.get('is_daytime', True)
        ...
        st.metric("🌡️ Current Conditions", f"{temp}°C", time_status)
        st.markdown(f"Clouds: {weather_data.get('cloud_cover', 0)}%")
