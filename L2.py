...
def clear_history():
    ...
    # Reset machine stop log (optional - comment out if you want to keep)
    # c.execute("DELETE FROM machine_stop_log")
...
def complete_maintenance(maintenance_id):
    """Mark a maintenance task as completed and remove it"""
    conn = sqlite3.connect('industrial_monitoring.db')
    c = conn.cursor()

    # Mark as completed
    c.execute("UPDATE maintenance_schedule SET completed = ? WHERE id = ?", (True, maintenance_id))
...
def load_and_prepare_data():
    ...
    try:
        ...
        # Convert timestamp string back to datetime - handle microseconds properly
        if len(db_data) > 0 and 'timestamp' in db_data.columns:
            # Clean timestamp strings - remove microseconds if present
            db_data['timestamp'] = db_data['timestamp'].astype(str).str.split('.').str[0]
...
def add_sensor_data_to_db(data_row):
    ...
    # Convert timestamp to string to avoid type issues - remove microseconds
    timestamp_val = data_row.get('timestamp', datetime.now())
    if isinstance(timestamp_val, datetime):
        timestamp_str = timestamp_val.strftime('%Y-%m-%d %H:%M:%S')
    else:
        timestamp_str = str(timestamp_val).split('.')[0]  # Remove microseconds
...
def data_upload_panel():
    ...
    with st.sidebar.expander("📂 Data Management", expanded=False):
        ...
        if st.button("🔄 Generate Sample Dataset (5000 records)", width='stretch'):
        ...
        if st.button("🗑️ Clear All Sensor Data", width='stretch'):
            if clear_history():
                st.success("✅ All sensor data has been cleared!")
                st.rerun()
            else:
        ...
        if uploaded_file is not None:
            try:
                ...
                if missing_cols:
                    ...
                    if st.button("📥 Load Dataset for Training", width='stretch'):
                            ...
                            for _, row in df.iterrows():
                                ...
                                if 'timestamp' in row_dict:
                                    if isinstance(row_dict['timestamp'], datetime):
                                        ...
                                        row_dict['timestamp'] = str(row_dict['timestamp']).split('.')[0]
                                else:
                                    row_dict['timestamp'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        ...
        if st.button("🔄 Refresh Data Status", width='stretch'):
            st.rerun()
...
def main():
    ...
    if not st.session_state.machine_stopped:
        if st.sidebar.button("🛑 EMERGENCY STOP MACHINE", key="stop_machine_btn", width='stretch'):
        ...
        if st.sidebar.button("🟢 RESTART MACHINE", key="restart_machine_btn", width='stretch'):
            st.session_state.machine_stopped = False
            st.session_state.critical_mode = False
            st.session_state.stop_machine_time = None
                ...
                if predictions:
                    ...
                    with col1:
                        ...
                        fig_ttf.update_layout(height=300, paper_bgcolor="rgba(0,0,0,0)",
                                              font={'color': 'white'})
                        st.plotly_chart(fig_ttf, width='stretch')
        ...
        if len(historical_df) > 0:
            ...
            with col1:
                ...
                fig_likelihood.update_layout(height=350, paper_bgcolor="rgba(0,0,0,0)",
                                             font={'color': 'white'})
                st.plotly_chart(fig_likelihood, width='stretch')
    ...
    # ========================================================================
        ...
        # Maintenance scheduler - WITH EMAIL ON SCHEDULE AND COMPLETION BUTTON
        ...
        # Display scheduled maintenance with COMPLETE buttons
        try:
            scheduled_df = pd.read_sql_query(
                "SELECT * FROM maintenance_schedule WHERE completed = 0 ORDER BY scheduled_date ASC LIMIT 20", conn)
            if len(scheduled_df) > 0:
                ...
                for idx, row in scheduled_df.iterrows():
                    ...
                    with col3:
                        st.write(f"Scheduled: {row['scheduled_date']}")
                    with col4:
                        st.write(f"Scheduled: {row['timestamp'][:10] if row['timestamp'] else 'N/A'}")
                    with col5:
                        if st.button(f"✅ Complete", key=f"complete_{row['id']}"):
                            complete_maintenance(row['id'])
                            st.success(f"✅ Maintenance '{row['task']}' marked as completed!")
                            st.rerun()
                    st.divider()
            else:
                st.info("No pending maintenance tasks. All caught up!")
        except Exception as e:
        ...
        try:
            completed_df = pd.read_sql_query(
                "SELECT * FROM maintenance_schedule WHERE completed = 1 ORDER BY scheduled_date DESC LIMIT 10", conn)
            if len(completed_df) > 0:
                with st.expander("📋 Completed Maintenance History"):
                    st.dataframe(completed_df[['task', 'priority', 'scheduled_date', 'timestamp']], width='stretch')
        except:
            pass
        ...
        if len(historical_df) > 0:
            ...
            try:
                ...
                if len(stop_log_df) > 0:
                    ...
                    st.dataframe(stop_log_df[['timestamp', 'reason', 'restored_at']], width='stretch')
            except:
                pass
